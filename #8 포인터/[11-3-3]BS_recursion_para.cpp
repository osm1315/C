...
{	int mid;
    ...
    *index=-1;
	...
	{   mid=(start+last)/2;
		(*count)++;
		if(target>pool[mid]) 	binary_search(pool, target, start, mid-1, count,index);
		else if(target<pool[mid]) 	binary_search(pool, target, mid+1, last, count,index);
		else *index= mid;     //FOUND       //return mid(NOT working)
	}
//return index;    //NOT Found				//return -1(NOT working)
return;
}
